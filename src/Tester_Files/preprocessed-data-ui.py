...
# These should point to your raw data files.
# The paths should be relative to where you run this script from.
# It's best to use full paths or ensure you're in the correct directory.
from src.config.config import THEMATIC_STORIES, CACHED_CLUSTER_STORIES, CACHED_STORIES_PATH, PICKEL_FILE

# Define where to save the fast, pre-computed files
OUTPUT_DIR = PICKEL_FILE
...
def precompute_conversation_data():
    ...
    output_path = os.path.join(OUTPUT_DIR, 'conversation_df.pkl')
...
def precompute_narrative_data():
    ...
    output_path = os.path.join(OUTPUT_DIR, 'narratives_dict.pkl')
    with open(output_path, 'wb') as f:
...
def precompute_activity_spike_data():
    ...
    output_path_list = os.path.join(OUTPUT_DIR, 'activity_spikes.pkl')
    output_path_map = os.path.join(OUTPUT_DIR, 'activity_spike_map.pkl')
...
# --- Run All Pre-computation Steps ---
if __name__ == '__main__':
    precompute_conversation_data()
    precompute_narrative_data()
    precompute_activity_spike_data()
    print("\n🎉 All data has been pre-computed and saved.")