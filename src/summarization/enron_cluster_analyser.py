...
from src.config.config import CLUSTER_STORIES
...
class EfficientEnronAnalyzer:
        ...
        for burst_date in burst_dates:
            ...
            if len(burst_emails) >= 5:
                story = {
                    ...
                    'duration_days': 3,
                    'summary': summarize_cluster_emails(burst_emails,
                                                        context="burst on " + burst_date.strftime('%Y-%m-%d')),
    ...
    def cluster_emails_semantically(self):
        if self.emails is None or self.emails.empty:
            st.warning("No emails loaded to cluster.")
            return None, None, None

        print("🔄 Preparing text for embedding...")
        self.emails['text_for_clustering'] = self.emails.apply(
            lambda row: f"{row.get('subject', '')} {row.get('summary', '')}", axis=1
        )

        progress = st.progress(0, text="🔄 Starting clustering pipeline...")

        print("🔍 Generating sentence embeddings using MiniLM...")
        progress.progress(20, "🔍 Generating embeddings with MiniLM...")
        model = SentenceTransformer('all-MiniLM-L6-v2')
        embeddings = model.encode(self.emails['text_for_clustering'].tolist(), show_progress_bar=True)

        print("📉 Reducing dimensions with UMAP...")
        progress.progress(50, "📉 Reducing dimensionality with UMAP...")
        reducer = umap.UMAP(n_neighbors=15, min_dist=0.1, metric='cosine', random_state=42)
        reduced = reducer.fit_transform(embeddings)

        print("📦 Clustering with HDBSCAN...")
        progress.progress(75, "📦 Clustering with HDBSCAN...")
        clusterer = hdbscan.HDBSCAN(min_cluster_size=5, prediction_data=True)
        labels = clusterer.fit_predict(reduced)

        print(f"✅ Done! Found {len(set(labels)) - (1 if -1 in labels else 0)} clusters (excluding noise).")
        progress.progress(100, "✅ Clustering complete.")

        self.emails['cluster'] = labels
        self.clustered_embeddings = reduced
        self.cluster_labels = labels

        return reduced, labels, self.emails

    def summarize_clusters_to_json(self, model='mistral', output_file='clustered_stories.json'):
        if 'cluster' not in self.emails.columns:
            st.warning("Run clustering before generating cluster summaries.")
            return

        clustered_stories = []
        for cluster_id in sorted(set(self.emails['cluster'])):
            if cluster_id == -1:
                continue  # Skip noise
            cluster_df = self.emails[self.emails['cluster'] == cluster_id]
            email_texts = cluster_df['text_for_clustering'].tolist()[:40]
            prompt = (
        ...
        try:
            with open(output_file, 'w', encoding='utf-8') as f:
                json.dump(clustered_stories, f, indent=2)
            print(f"✅ Cluster summaries saved to {output_file}")
        except Exception as e:
...
def main():
    ...
    if analyzer.emails is not None:
        ...
        with st.expander("📌 Semantic Clusters (UMAP + HDBSCAN)"):
            if st.button("Run Clustering"):
                with st.spinner("Embedding and clustering emails..."):
                    reduced, labels, clustered_df = analyzer.cluster_emails_semantically()
                    if reduced is not None:
                        cluster_df = pd.DataFrame(reduced, columns=["x", "y"])
                        cluster_df['label'] = labels
                        cluster_df['text'] = analyzer.emails['text_for_clustering']

                        fig = px.scatter(cluster_df, x='x', y='y', color=cluster_df['label'].astype(str),
                                     hover_data=['text'], title="Semantic Clusters of Emails")
                        st.plotly_chart(fig, use_container_width=True)

                        if st.button("📝 Generate Cluster Summaries"):
                            with st.spinner("Summarizing clusters with Mistral via Ollama..."):
                                analyzer.summarize_clusters_to_json()
                                st.success("✅ Cluster summaries saved to `clustered_stories.json`")
