...
from src.config.config import PROCESSED_JSON_OUTPUT,CLUSTER_STORIES,EMBEDDING_CACHE_FILE
...
def generate_cluster_summaries(df):
    ...
    with open(CLUSTER_STORIES, 'w', encoding='utf-8') as f:
        json.dump(summaries, f, indent=2)
    return summaries

def load_cached_summaries():
    if os.path.exists(CLUSTER_STORIES):
